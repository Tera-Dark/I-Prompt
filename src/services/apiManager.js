...
class ApiManager {
   ...
   */
  async init() {
    logger.api('🚀 [ApiManager] 初始化API管理器...');
    ...
    if (zhipuApi) {
      ...
      if (zhipuConfig.apiKey) {
        ...
        logger.api('🔑 [ApiManager] 已为智谱GLM设置API密钥');
      } else {
        logger.warn('⚠️ [ApiManager] 智谱GLM未设置API密钥');
      }
    }
    ...
    logger.api(`✅ [ApiManager] 初始化完成，当前API: ${this.currentApi?.name || 'None'}`);
  }
  ...
  async checkApiHealth(api) {
    ...
    try {
      logger.api(`🔍 [ApiManager] 检查API健康状态: ${api.name}`);
      
      // 如果是智谱GLM且需要API密钥但未设置，跳过检查
      if (api.provider === 'zhipu' && api.requiresApiKey && !api.apiKey) {
        logger.warn(`⚠️ [ApiManager] ${api.name} 需要设置API密钥，跳过健康检查`);
        api.available = false;
      ...
      logger.api(`${isHealthy ? '✅' : '❌'} [ApiManager] ${api.name} 健康检查完成: ${responseTime}ms, 状态: ${response.status}`);
      ...
      logger.error(`❌ [ApiManager] ${api.name} 健康检查失败: ${error.message}`);
  ...
  async checkAllApisHealth() {
    logger.api('🔄 [ApiManager] 开始全面健康检查...');
    ...
    logger.api(`📊 [ApiManager] 健康检查完成，可用API: ${availableCount}/${this.apis.length}`);
  }
  ...
  async switchToNextAvailableApi() {
    ...
    if (availableApis.length === 0) {
      logger.warn('⚠️ [ApiManager] 没有可用的API');
      return false;
    }
    ...
    if (nextApi !== this.currentApi) {
      ...
      logger.api(`🔄 [ApiManager] API切换: ${oldApi} -> ${nextApi.name}`);
  ...
  async makeRequest(messages, options = {}) {
    ...
    logger.api(`📤 [ApiManager] 使用 ${api.name} 发送请求`);
    logger.api(`📋 [ApiManager] 请求体:`, JSON.stringify(requestBody, null, 2));
    ...
    try {
      ...
      logger.api(`📡 [ApiManager] 请求头:`, headers);
      ...
      logger.api(`📶 [ApiManager] 响应状态: ${response.status} ${response.statusText}`);
      ...
      if (!response.ok) {
        ...
        try {
          ...
          logger.error(`❌ [ApiManager] 错误响应内容:`, errorText);
        } catch (e) {
          logger.error(`❌ [ApiManager] 无法读取错误响应内容`);
        }
        ...
        if (switched && this.currentApi !== api) {
          logger.api(`🔄 [ApiManager] 请求失败，已切换到 ${this.currentApi.name}，重试请求`);
      ...
      logger.api(`📥 [ApiManager] 响应数据:`, JSON.stringify(data, null, 2));
      
      // 检查响应内容是否有效
      if (!data.choices || !data.choices[0] || !data.choices[0].message) {
        logger.warn(`⚠️ [ApiManager] ${api.name} 返回了无效的响应结构`);
        throw new Error('API返回了无效的响应结构');
      }

      const content = data.choices[0].message.content;
      if (!content || content.trim().length === 0) {
        logger.warn(`⚠️ [ApiManager] ${api.name} 返回了空内容`);
        throw new Error('API返回了空内容');
      }

      logger.api(`✅ [ApiManager] ${api.name} 请求成功，内容长度: ${content.length}`);
      
      return data;

    } catch (error) {
      logger.error(`❌ [ApiManager] ${api.name} 请求失败:`, error);
      ...
      const switched = await this.switchToNextAvailableApi();
      if (switched && this.currentApi !== api) {
        logger.api(`🔄 [ApiManager] 请求错误，已切换到 ${this.currentApi.name}，重试请求`);
        return this.makeRequest(messages, options); // 递归重试
      }
  ...
  startHealthCheck() {
    ...
    logger.api('🩺 [ApiManager] 健康检查服务已启动');
  }
  ...
  stopHealthCheck() {
    if (this.healthCheckInterval) {
      clearInterval(this.healthCheckInterval);
      this.healthCheckInterval = null;
      logger.api('🛑 [ApiManager] 健康检查服务已停止');
    }
  }
   ...
   */
  async refreshApis() {
    logger.api('🔄 [ApiManager] 手动刷新API状态...');
    await this.checkAllApisHealth();
  }
  ...
  setPreferredApi(targetApi) {
    ...
    if (foundApi) {
      ...
      logger.api(`🎯 [ApiManager] 手动切换API: ${oldApi} -> ${foundApi.name}`);
  ...
  async updateZhipuApiKey() {
    ...
    if (zhipuApi) {
      ...
      if (zhipuConfig.apiKey) {
        zhipuApi.apiKey = zhipuConfig.apiKey;
        logger.api('🔑 [ApiManager] 智谱GLM API密钥已更新');
        ...
        logger.warn('⚠️ [ApiManager] 智谱GLM API密钥已清除');
        return false;
      }
    }
    return false;
